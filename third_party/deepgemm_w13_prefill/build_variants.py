#!/usr/bin/env python3
"""Reproducibly build stock, PSUM-control, and XOR PSUM DeepGEMM modules."""
...
DEFAULT_OUTPUT = Path(
    ...
    "28-moe_w13_prefill_psum/deepgemm/w13_prefill_variants"
)
HERE = Path(__file__).resolve().parent
PATCH = HERE / "patches" / "0001-psum-utccp-xor-transpose.patch"
...
# Filled from two independent git-archive reconstructions. These values are
...
EXPECTED_SOURCE_IDENTITY = {
    ...
    "complete_source_diff_sha256": (
        "e5d75cf8116a3497e5677ab28e2800dda3af2a78b977092417c33b039735e134"
    ),
}
...
def _load_helper() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "_glm52_deepgemm_build_helper", HELPER_PATH
    )
...
def verify_source(upstream: Path) -> None:
    ...
    actual_blobs = {
        relative: _git_blob_sha256(upstream, relative)
    ...
    if not PATCH.is_file():
        raise RuntimeError(f"missing tracked XOR patch: {PATCH}")
...
def reconstruct_identity(upstream: Path, scratch_parent: Path) -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(
        dir=scratch_parent, prefix=".w13-prefill-materialization."
    ) as temporary:
        root = Path(temporary)
        for pass_index in range(2):
            stock = materialize_source(
                upstream, root / f"stock-{pass_index}", apply_xor_patch=False
            )
            psum = materialize_source(
                upstream, root / f"psum-{pass_index}", apply_xor_patch=False
            )
            xor = materialize_source(
                upstream, root / f"xor-{pass_index}", apply_xor_patch=True
            )
            if stock != psum:
                raise RuntimeError(
                    "stock and PSUM control source trees are not byte-identical"
                )
            if xor == stock:
                raise RuntimeError("XOR patch did not alter the source tree")
            passes.append(
                {
                    "stock_source_tree_sha256": stock,
                    "psum_source_tree_sha256": psum,
                    "xor_source_tree_sha256": xor,
...
def main() -> int:
    ...
    for name, patched in (("stock", False), ("psum", False), ("xor", True)):
        ...
        package = (
            ...
            / name
            / "site"
            / f"deep_gemm_w13_prefill_{name}"
        )
        build_directory = output / "compile" / name
        if build_directory.exists():
            shutil.rmtree(build_directory)
        source_digest = materialize_source(
            upstream, source, apply_xor_patch=patched
        )
        H.copy_package_source(source, package)
        shared_object = H.build_extension(
            source, package, build_directory
        ).resolve()
    ...
    manifest = {
        ...
        "build": {
            ...
            "package_template": (
                "<OUTPUT>/artifacts/<VARIANT>/site/"
                "deep_gemm_w13_prefill_<VARIANT>"
            ),
