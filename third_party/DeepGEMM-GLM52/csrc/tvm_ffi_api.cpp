...
void dg_fp8_fp4_gemm_nt_task08_gated_dual(
        TensorView a, TensorView a_sf,
        TensorView b, TensorView b_sf,
        TensorView d) {
    gemm::fp8_fp4_gemm_nt_task08_gated_dual(
        std::make_pair(convert_to_torch_tensor(a), convert_to_torch_tensor(a_sf)),
        std::make_pair(convert_to_torch_tensor(b), convert_to_torch_tensor(b_sf)),
        convert_to_torch_tensor(d));
}
...
void dg_fp8_fp4_gemm_nt_fused(TensorView a, TensorView a_sf,
                              ...
                              Optional<TensorView> c,
                              std::string compiled_dims,
                              Optional<TensorView> prof) {
    auto c_opt = c.has_value()? std::optional<torch::Tensor>(convert_to_torch_tensor(c.value())) : std::nullopt;
    auto prof_opt = prof.has_value()? std::optional<torch::Tensor>(convert_to_torch_tensor(prof.value())) : std::nullopt;
    gemm::fp8_fp4_gemm_nt_fused(std::make_pair(convert_to_torch_tensor(a), convert_to_torch_tensor(a_sf)),
                                std::make_pair(convert_to_torch_tensor(b), convert_to_torch_tensor(b_sf)),
...
void dg_fp8_fp4_gemm_nt_prof(TensorView a, TensorView a_sf,
                             ...
                             std::string compiled_dims,
                             TensorView prof,
                             bool fuse_scale_pack) {
    auto c_opt = c.has_value()? std::optional<torch::Tensor>(convert_to_torch_tensor(c.value())) : std::nullopt;
    gemm::fp8_fp4_gemm_nt_prof(std::make_pair(convert_to_torch_tensor(a), convert_to_torch_tensor(a_sf)),
                               std::make_pair(convert_to_torch_tensor(b), convert_to_torch_tensor(b_sf)),
                               convert_to_torch_tensor(d), c_opt, compiled_dims,
                               convert_to_torch_tensor(prof), fuse_scale_pack);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(fp8_fp4_gemm_nt_task08_gated_dual, dg_fp8_fp4_gemm_nt_task08_gated_dual);
