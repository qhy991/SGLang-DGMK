...
# Matched exact-x1 Nsight attribution for source control vs direct multimem.
# No profiler timing is used for promotion; the five-pair no-profiler screen is
...
CANDIDATE_CONTRACT=$EVIDENCE/runtime_source_contract_cp8_direct_packed_kv_final.json
...
OUT=$EVIDENCE/direct_packed_matched_nsys_$STAMP
mkdir -p "$OUT"
...
run_arm() {
  local label=$1 repo=$2 contract=$3 env_file=$4 map log report trigger
  ...
  echo "[PASS] $label report=$report.nsys-rep"
}

run_arm control "$CONTROL_REPO" "$CONTROL_CONTRACT" "$CONTROL_ENV"
run_arm direct "$CANDIDATE_REPO" "$CANDIDATE_CONTRACT" "$CANDIDATE_ENV"
echo "[DONE] $OUT"
