...
# Five adjacent fresh-server pairs for c01 versus the default-off CP8
# per-forward FlashMLA metadata-reuse candidate.
...
SUMMARIZER=$CANDIDATE_REPO/glm52_opt/scripts/summarize_cp8_metadata_reuse_five_pairs.py
...
OUT=$EVIDENCE/metadata_reuse_five_pairs_$STAMP
mkdir -p "$OUT"
...
run_arm() {
  local label=$1 arm=$2 repo contract env_file map log correctness_json
  ...
  [[ -f "$result_json" && -f "$correctness_json" && -f "$server_log" ]] || {
    echo "[ERR] missing artifacts for $label/$arm" >&2
    return 2
  }
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
...
run_arm p1a baseline
run_arm p1b candidate
run_arm p2a candidate
run_arm p2b baseline
run_arm p3a baseline
run_arm p3b candidate
run_arm p4a candidate
run_arm p4b baseline
run_arm p5a baseline
run_arm p5b candidate
