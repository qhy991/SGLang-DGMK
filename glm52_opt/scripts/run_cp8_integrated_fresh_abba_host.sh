...
# Five adjacent fresh-server pairs for the integrated CP8 prefill candidate.
# Each arm runs one exact-input x11 point and the frozen 11-input logprob probe.
...
INTEGRATED_REPO=$ROOT/SGLang-DGMK-cp8-packed-kv-comm-20260818
LAUNCHER=$INTEGRATED_REPO/glm52_opt/scripts/run_glm52_100k_cp8_ep8_packed_review_host.sh
RUNNER=$INTEGRATED_REPO/glm52_opt/scripts/run_glm52_100k_prefill_cp8_exact_concurrent.py
CONTROL_CONTRACT=$EVIDENCE/runtime_source_contract_cp8_packed_kv_control.json
INTEGRATED_CONTRACT=$EVIDENCE/runtime_source_contract_cp8_integrated.json
CONTROL_ENV=$EVIDENCE/host_e2e_prefill_cp8_packed_kv_baseline.env
INTEGRATED_ENV=$INTEGRATED_REPO/glm52_opt/host_e2e_prefill_cp8_integrated.env
...
OUT=$EVIDENCE/integrated_prefill_fresh_abba_$STAMP
mkdir -p "$OUT"
...
run_arm() {
  local label=$1 arm=$2 repo contract env_file log static_map expected_commit
  local correctness_json
  if [[ "$arm" == control ]]; then
    repo=$CONTROL_REPO
    contract=$CONTROL_CONTRACT
    env_file=$CONTROL_ENV
  elif [[ "$arm" == integrated ]]; then
    repo=$INTEGRATED_REPO
    contract=$INTEGRATED_CONTRACT
    env_file=$INTEGRATED_ENV
  else
  ...
  wait_clear
  echo "[RUN] label=$label arm=$arm commit=$expected_commit log=$log"
  env \
  ...
  [[ -f "$result_json" && -f "$correctness_json" && -f "$server_log" ]] || {
    echo "[ERR] arm artifacts missing: result=$result_json correctness=$correctness_json server=$server_log" >&2
    return 2
  }
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
...
run_arm p1a control
run_arm p1b integrated
run_arm p2a integrated
run_arm p2b control
run_arm p3a control
run_arm p3b integrated
run_arm p4a integrated
run_arm p4b control
run_arm p5a control
run_arm p5b integrated

python3 "$INTEGRATED_REPO/glm52_opt/scripts/summarize_cp8_integrated_fresh_abba.py" \
  --manifest "$MANIFEST" --output "$OUT/summary.json"
