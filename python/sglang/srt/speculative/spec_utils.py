...
def traverse_tree(
    ...
    Returns:
        A tuple of (direct_rejected, pruned_rejected) for single-chain speculative
        decoding. Tree/topk cases keep the counters at zero to avoid mixing
        different rejection semantics.
    """
    assert (
        retrieve_next_token.shape == retrieve_next_sibling.shape == draft_tokens.shape
    )

    is_single_chain = not bool(torch.any(retrieve_next_sibling != -1).item())
    direct_rejected = 0
    pruned_rejected = 0

    def is_token_allowed(parent_bitmask: torch.Tensor, token_id: int) -> bool:
        if token_id < 0:
            return False
        if vocab_size and token_id >= vocab_size:
            return False
        return (
            int(parent_bitmask[token_id // 32].item()) & (1 << (token_id % 32))
        ) != 0

    def count_chain_descendants(curr: int) -> int:
        count = 0
        next_pos = int(retrieve_next_token[curr].item())
        max_steps = retrieve_next_token.numel()
        while next_pos != -1 and 0 <= next_pos < max_steps and count < max_steps:
            count += 1
            next_pos = int(retrieve_next_token[next_pos].item())
        return count
    ...
    def dfs(
        ...
        nonlocal direct_rejected, pruned_rejected
        if curr == 0:
            ...
            curr_token_id = int(draft_tokens[curr].item())
            accepted = is_token_allowed(parent_bitmask, curr_token_id)
            if not accepted and is_single_chain and (
                vocab_size is None or curr_token_id < vocab_size
            ):
                direct_rejected += 1
                pruned_rejected += count_chain_descendants(curr)
    ...
    return direct_rejected, pruned_rejected
...
def generate_token_bitmask(
    ...
    for i, req in enumerate(reqs):
        if req.grammar is not None:
            ...
            direct_rejected += direct
            pruned_rejected += pruned
    ...
    return allocate_token_bitmask, direct_rejected, pruned_rejected
