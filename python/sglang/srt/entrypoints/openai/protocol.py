...
class ChatCompletionMessageGenericParam(BaseModel):
    ...
    def _normalize_role(cls, v):
        if isinstance(v, str):
            v_lower = v.lower()
            if v_lower not in {"system", "assistant", "tool", "function", "developer", "_input"}:
                raise ValueError(
                    "'role' must be one of 'system', 'developer', 'assistant', 'tool', 'function', or '_input' (case-insensitive)."
                )
            return v_lower
