...
_EXPERTS: Final = 32
_B200_EXPERT_SLAB: Final = 1024
_B300_EXPERT_SLAB: Final = 8192
_SUPPORTED_CONTRACTS: Final = frozenset(
    {
        ((10, 0), _B200_EXPERT_SLAB),
        ((10, 3), _B300_EXPERT_SLAB),
    }
)
_GATE_UP: Final = 4096
_HIDDEN: Final = 2048
...
def _log_selected_contract(
    ...
    logger.warning(
        "GLM-5.2 masked SwiGLU quant selected: variant=%s capability=%s "
        "shape=%s stride=%s routed_m=%d topk=%d",
        variant,
        capability,
        shape,
        stride,
...
def _eligibility_error(
    ...
    *,
    ...
    capability = _device_capability(gateup_output.device.index or 0)
    if gateup_output.dtype != torch.bfloat16:
        return "gateup_output must be bfloat16"
    if gateup_output.ndim != 3:
        return "gateup_output must be rank 3"
    expert_slab = int(gateup_output.shape[1])
    if (capability, expert_slab) not in _SUPPORTED_CONTRACTS:
        return (
            "unsupported GPU/expert-slab contract: "
            f"capability={capability}, expert_slab={expert_slab}"
        )
    if tuple(gateup_output.shape) != (_EXPERTS, expert_slab, _GATE_UP):
        return (
            "gateup_output must have exact shape "
            f"{(_EXPERTS, expert_slab, _GATE_UP)}"
        )
...
def _allocate_outputs(
    ...
    expert_slab = int(gateup_output.shape[1])
    output = torch.empty(
        (_EXPERTS, expert_slab, _HIDDEN),
        device=gateup_output.device,
        dtype=torch.float8_e4m3fn,
    )
    scale_storage = torch.empty(
        (_EXPERTS, _PACKED_GROUPS, expert_slab),
        device=gateup_output.device,
        dtype=torch.int32,
    )
...
def silu_mul_quant_packed_into(
    ...
    *,
    ...
    expert_slab = int(gateup_output.shape[1])
    if tuple(output.shape) != (_EXPERTS, expert_slab, _HIDDEN):
        raise ValueError(f"output must have shape [32, {expert_slab}, 2048]")
    ...
    if tuple(scale_storage.shape) != (
        ...
        raise ValueError(
            f"scale_storage must have physical shape [32, 4, {expert_slab}]"
        )
...
def maybe_silu_mul_quant_packed(
    ...
    *,
    ...
    if error is not None:
        return None
    assert num_real_tokens is not None
    _log_selected_contract(
        _device_capability(gateup_output.device.index or 0),
        tuple(gateup_output.shape),
        tuple(gateup_output.stride()),
        num_real_tokens,
        variant,
    )
