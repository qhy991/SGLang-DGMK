...
_OVERLAY_DIR = _REPO_ROOT / "third_party" / "deepgemm_w2_em8_bm16_stage11"
...
CANDIDATE_IMPORT_NAME = "deep_gemm_glm52_w2_em8_bm16_stage11_v3"
BUILD_ID = (
...
JIT_IDENTITY = "sm100_m_grouped_fp8_fp4_gemm_masked_1d1d_glm52_w2_em8_bm16_stage11_v3"
FORWARD_BUCKET = (32, 8)
...
def expected_manifest_path() -> Path:
    source_sha = _sha256(_OVERLAY_DIR / "source.patch")
    build_tool_sha = _sha256(_OVERLAY_DIR / "build_tool.patch")
    build_key = f"{BASE_COMMIT[:12]}-{source_sha[:12]}-{build_tool_sha[:12]}"
    return (
        _REPO_ROOT
        / "build"
        / "deepgemm-w2-em8-bm16-stage11-v3-overlays"
        / build_key
        / "manifest.json"
    ).resolve()
...
def _verify_manifest() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            "em8/BM16/stage11 overlay manifest missing; run the released "
            f"{_OVERLAY_DIR / 'build_overlay.sh'}: {path}"
        )
    completed = subprocess.run(
        [
            sys.executable,
            str(_OVERLAY_DIR / "overlay_manifest.py"),
            "verify",
            "--manifest",
            str(path),
            "--check-env",
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"em8/BM16/stage11 overlay verification failed: {detail}")
    return path
...
def _load_candidate(manifest: dict[str, Any], stock: Any) -> Any:
    ...
    actual_build_id = getattr(
        module,
        "GLM52_W2_EM8_BM16_STAGE11_V3_BUILD_ID",
        None,
    )
...
def request() -> None:
    global _REQUESTED, _PREPARED, _PREPARE_ERROR
    _REQUESTED = True
    _PREPARED = None
    _PREPARE_ERROR = None
...
def create_layer_contract(
    *,
    ...
    if not eligible:
        raise RuntimeError(
            "explicit em8/BM16/stage11-v3 profile rejected the W2 layer/runtime ABI"
        )
...
def prepare_callsite_contract(
    ...
    *,
    ...
    if contract is None:
        raise RuntimeError("stage11-v3 layer contract is missing")
    expected_m = _strict_int(expected_m, "expected_m")
    if expected_m != 8:
        return False
    runtime = contract.runtime
    if runtime is None or runtime is not _PREPARED:
        raise RuntimeError("stage11-v3 prepared runtime token is missing or stale")
    state = runtime.current_forward_state()
    if state is None:
        raise RuntimeError("stage11-v3 forward context is missing")
    ...
    if recipe_a is not None or recipe_b is not None or overlap_args is not None:
        raise RuntimeError(
            "exact M32/em8 stage11-v3 route does not support recipe or overlap"
        )
    if contract.callsite_checked:
        if not contract.callsite_eligible:
            raise RuntimeError("exact M32/em8 stage11-v3 ABI was previously rejected")
        return True
    ...
    if not eligible:
        raise RuntimeError("explicit M32/em8 stage11-v3 callsite ABI rejected")
    return True
