...
class DeepGemmKernelType(IntEnum):
    ...
    GEMM_NT_F8F8BF16_COMPILED_NK = auto()
...
def _maybe_compile_deep_gemm_one_type_all(
    ...
    if (
        ...
        # The GLM-5.2 fixed-N/K specialization has a distinct JIT/cache
        # identity and exactly two supported local token buckets. Do not grow
        # the ordinary broad-M precompile portfolio for this narrow lane.
        m_list = (
            [16, 32]
            if kernel_type == DeepGemmKernelType.GEMM_NT_F8F8BF16_COMPILED_NK
...
class _BaseWarmupExecutor:
    ...
    def create(kernel_type: DeepGemmKernelType, **kwargs):
        return {
            ...
            DeepGemmKernelType.GEMM_NT_F8F8BF16_COMPILED_NK: _CompiledNkWarmupExecutor,
    ...
    def get_memory_requirement(
        ...
        if kernel_type in (
            ...
            DeepGemmKernelType.GEMM_NT_F8F8BF16_COMPILED_NK,
        ):
...
class _CompiledNkWarmupExecutor(_BaseWarmupExecutor):
    """Prewarm the two packed-UE8M0 GLM O-projection compiled-N/K keys."""

    _SUPPORTED_M = (16, 32)

    def __init__(self, max_m: int, n: int, k: int, num_groups: int):
        assert max_m == 32 and n == 6144 and k == 16384 and num_groups == 1
        packed_k = ceil_div(k, _BLOCK_SIZE * 4)
        self.lhs_q = torch.empty((max_m, k), device="cuda", dtype=torch.float8_e4m3fn)
        self.lhs_s = {
            m: torch.ones((packed_k, m), device="cuda", dtype=torch.int32).T
            for m in self._SUPPORTED_M
        }
        self.rhs_q = torch.empty((n, k), device="cuda", dtype=torch.float8_e4m3fn)
        self.rhs_s = torch.ones((packed_k, n), device="cuda", dtype=torch.int32).T
        self.out = torch.empty((max_m, n), device="cuda", dtype=torch.bfloat16)
