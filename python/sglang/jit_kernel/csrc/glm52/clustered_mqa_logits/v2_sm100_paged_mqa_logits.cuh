...
template <uint32_t kNextN, bool kIsContextLens2D, bool kIsVarlen,
          uint32_t kNumHeads, uint32_t SPLIT_KV, uint32_t PAGE_KV,
          uint32_t kSplitsPerChunk, uint32_t kBlockQ = 8>
struct SM100PagedMQALogitsScheduler : SM100IndicesStorage<kIsVarlen> {
    ...
    uint32_t cluster_rank;              // CTA rank selects one of the two Q4 blocks
    ...
    uint32_t cur_request_q_token_start; // request base for Q16 weights/output
    ...
    CUTLASS_DEVICE SM100PagedMQALogitsScheduler(const uint32_t& sm_idx,
        ...
        this->cluster_rank = cluster_rank;
        if constexpr (kIsVarlen)
        ...
        cur_q_block_in_request = cluster_rank;
    ...
    CUTLASS_DEVICE bool next_q_block(uint32_t& q_block_idx, uint32_t& kv_split_base, uint32_t& num_kv_splits) {
        ...
        cur_request_q_token_start = cur.q_token_start;
        ...
        // The two clustered CTAs advance through identical KV chunks.  Each
        // CTA emits only its rank-selected Q4 block, so a multicast KV stage
        // is consumed once by both CTAs without serializing the two blocks.
        cur_kv_split_base += num_kv_splits;
        if (cur_kv_split_base >= upper) {
            if (cur.q_token_start == end_q_token_idx) {
                // Reached the next cluster's start.
                done = true;
            } else {
                ...
                if (next_q_token >= num_q_tokens_total)
                    ...
                    cur_q_block_in_request = cluster_rank;
                    // The new request may already be this cluster's end.
    ...
    CUTLASS_DEVICE uint32_t get_cluster_q_tma_token_base(const uint32_t&) const {
        return cur_request_q_token_start;
    }
    ...
    CUTLASS_DEVICE uint32_t get_cluster_logits_row(const uint32_t& token_idx) const {
        return cur_request_q_token_start + token_idx;
    }
