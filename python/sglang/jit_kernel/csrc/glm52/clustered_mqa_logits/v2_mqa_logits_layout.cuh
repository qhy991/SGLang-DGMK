...
struct MQALogitsSharedStorage {
    ...
    using PeerBarrier = cutlass::arch::ClusterBarrier;
    ...
    // cta_group::2 owns one M128 half of the logical M256 KV tile per CTA.
    // The two halves live at identical local SMEM offsets and are gathered by
    // tcgen05.mma.cta_group::2; no multicast duplicate is materialized.
    static constexpr uint32_t kLocalSplitKV = SPLIT_KV / 2;
    static constexpr uint32_t kNumKVBytesPerStage = kLocalSplitKV * kNumQKBytesPerToken;
    ...
    static constexpr uint32_t kNumScaleKV = kIsFP4 ? kNumSFKV : kLocalSplitKV;
    ...
    static constexpr uint32_t kNumWeightsElementsPerStage =
        math::constexpr_align(kQueriesPerCluster * kNumHeads * static_cast<uint32_t>(sizeof(reduce_dtype_t)), kWeightsTmaAlignment)
        / static_cast<uint32_t>(sizeof(reduce_dtype_t));

    DG_STATIC_ASSERT(SPLIT_KV % 2 == 0, "cta_group::2 requires an even KV split");
    ...
    PeerBarrier q_peer_full_barriers[kNumQStages];
    PeerBarrier kv_peer_full_barriers[kNumKVStages];
    Barrier full_tmem_barriers[kNumTmemStages];
    Barrier empty_tmem_barriers[kNumTmemStages];
    PeerBarrier tmem_peer_empty_barriers[kNumTmemStages];
