...
// Task 25 keeps the audited stock kernel body and packed-UE8M0 store exactly
// unchanged.  The production launcher uses the padded expert slab (1024) to
// size its grid, so almost every block scans masked_m and returns during
// M16/M32 decode.  This wrapper is deliberately restricted to those two
// host-known buckets and launches one block per routed assignment instead.
...
struct Task25SiluAndMulMaskedPostQuantKernel {
  ...
  run(const tvm::ffi::TensorView input,
    ...
    RuntimeCheck(T.unwrap() == 1024, "Task-25 candidate requires a 1024-row expert slab");
